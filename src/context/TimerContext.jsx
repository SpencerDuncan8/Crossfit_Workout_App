...
export const TimerProvider = ({ children }) => {
  const [timer, setTimer, clearTimer] = usePersistentState('blockfitTimerState', {
    ...
    recordedTime: null,
    isRecordable: false,
    tabata: { totalRounds: 0, currentRound: 0, isWorkPhase: true, work: 20, rest: 10, workoutType: 'tabata' }, // Add workoutType
  ...
  const startTimer = ({ type, duration = 0, tabataRounds = 8, tabataWork = 20, tabataRest = 10, totalLaps = 0, isRecordable = false }) => {
    setTimer(prev => {
      let newTimerState = { 
        ...
        recordedTime: null,
        isRecordable,
        // --- FIX: Pass the 'type' to the tabata object ---
        tabata: { totalRounds: tabataRounds, currentRound: 1, isWorkPhase: true, work: tabataWork, rest: tabataRest, workoutType: type }, 
      ...
      if (type === 'tabata' || type === 'intervals') newTimerState.time = tabataWork; // Use 'intervals' as well
  ...
  const stopTimer = () => setTimer(prev => ({ ...prev, isActive: false, laps: [], totalLaps: 0, recordedTime: null, isRecordable: false }));
